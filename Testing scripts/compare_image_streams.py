...
class CompareImageStreamsNode:
    def __init__(self):
        ...
        self.frame_count = 0
        self.last_fps_stamp = time.monotonic()
        self.fps = 0.0
        self.raw_msg_count = 0
        self.enh_msg_count = 0
        self.last_rate_stamp = time.monotonic()
        ...
        self.toggle_subs = []
        self.toggle_subs.append(rospy.Subscriber(self.toggle_topic, Bool, self.toggle_cb, callback_args=self.toggle_topic, queue_size=1))
        self.toggle_pub = rospy.Publisher(self.toggle_topic, Bool, queue_size=1, latch=True)

        self.button_rect = None  # (x1, y1, x2, y2)
        ...
        if inferred_toggle and inferred_toggle != self.toggle_topic:
            self.toggle_subs.append(rospy.Subscriber(inferred_toggle, Bool, self.toggle_cb, callback_args=inferred_toggle, queue_size=1))
    ...
    def infer_namespaced_toggle_topic(raw_topic):
        # A namespaced test topic can still infer its matching toggle topic.
    ...
    def publish_toggle(self, enabled):
        msg = Bool()
        msg.data = bool(enabled)
        self.toggle_pub.publish(msg)
        with self.lock:
            self.enhancement_on = bool(enabled)
            self.last_toggle_topic = "ui_button"
            self.last_toggle_time = time.monotonic()

    @staticmethod
    def draw_button(img, rect, text, enabled):
        x1, y1, x2, y2 = rect
        fill = (50, 160, 70) if enabled else (60, 60, 180)
        border = (230, 230, 230)
        cv2.rectangle(img, (x1, y1), (x2, y2), fill, -1)
        cv2.rectangle(img, (x1, y1), (x2, y2), border, 2)
        size = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.56, 2)[0]
        tx = x1 + max(8, (x2 - x1 - size[0]) // 2)
        ty = y1 + max(22, (y2 - y1 + size[1]) // 2)
        cv2.putText(img, text, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX, 0.56, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    def mouse_cb(self, event, x, y, _flags, _param):
        ...
        with self.lock:
            ...
            current = self.enhancement_on
        if rect is None:
            return
        x1, y1, x2, y2 = rect
        if x1 <= x <= x2 and y1 <= y <= y2:
            self.publish_toggle(not current)
    ...
    def draw_overlay(img, title, m=None, extra_lines=None):
        ...
        if extra_lines:
            for ln in extra_lines:
                cv2.putText(out, ln, (18, y), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (160, 255, 160), 1, cv2.LINE_AA)
                y += 22
    ...
    def spin(self):
        ...
        while not rospy.is_shutdown():
            ...
            if raw is None:
                raw = self.letterbox_notice((self.target_height, int(self.target_height * 4 / 3), 3), ["Waiting for raw image..."])
            ...
            self.frame_count += 1
            now = time.monotonic()
            elapsed = now - self.last_fps_stamp
            if elapsed >= 1.0:
                self.fps = self.frame_count / elapsed
                self.frame_count = 0
                self.last_fps_stamp = now
            with self.lock:
                rate_elapsed = now - self.last_rate_stamp
                if rate_elapsed >= 1.0:
            ...
            if toggle_time is None:
                ...
                age = time.monotonic() - toggle_time
            ...
            btn_y2 = btn_y1 + btn_h
            with self.lock:
                self.button_rect = (btn_x1, btn_y1, btn_x2, btn_y2)
            btn_label = "Enhancement: ON" if enhancement_on else "Enhancement: OFF"
            self.draw_button(combined, (btn_x1, btn_y1, btn_x2, btn_y2), btn_label, enhancement_on)
            ...
            if key == 27 or key == ord("q"):
                ...
                break
            if key == ord("t"):
                self.publish_toggle(not enhancement_on)
